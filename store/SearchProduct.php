...
if ($sort == "")
{
		$productsresult = mysql_query("SELECT * FROM Products WHERE productname LIKE '%".$keyword."%'");
}
else if ($sort == "priceasc")
{
		$productsresult = mysql_query("select * FROM Products WHERE productname LIKE '%".$keyword."%' ORDER BY price ASC");
}
else if ($sort == "pricedes")
{
		$productsresult = mysql_query("select * FROM Products WHERE productname LIKE '%".$keyword."%' ORDER BY price DESC");
}
else if ($sort == "alphabet")
{
		$productsresult = mysql_query("select * FROM Products WHERE productname LIKE '%".$keyword."%' ORDER BY productname ASC");
}
...
if(mysql_num_rows($productsresult)== 0){
   echo "No results have been found in our catalogue";
}
else {
...
Sort by:
				<a href="SearchProduct.php?keyword=<?php echo $keyword; ?>&sort=alphabet "><button type="button">Alphabetical</button></a>
				<a href="SearchProduct.php?keyword=<?php echo $keyword; ?>&sort=priceasc  "><button type="button">Price Ascending</button></a>
				<a href="SearchProduct.php?keyword=<?php echo $keyword; ?>&sort=pricedes "><button type="button">Price Descending</button></a>
